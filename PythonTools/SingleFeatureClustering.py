...
# create dir
mode = 0o666
path = "./output/K-Means-Feature-To-Feature-Compare_{}".format(input_data_type)
...
path_p = "./output/K-Means-Feature-To-Feature-Compare_{}/pId-Confidence".format(input_data_type)
if not os.path.exists(path_p):
    os.mkdir(path_p, mode)

path_p = "./output/K-Means-Feature-To-Feature-Compare_{}/Conscientious-pId".format(input_data_type)
if not os.path.exists(path_p):
    os.mkdir(path_p, mode)

path_p = "./output/K-Means-Feature-To-Feature-Compare_{}/Conscientious-pId-highest_confidet".format(input_data_type)
if not os.path.exists(path_p):
    os.mkdir(path_p, mode)

path_p = "./output/K-Means-Feature-To-Feature-Compare_{}/ROC-curve".format(input_data_type)
if not os.path.exists(path_p):
    os.mkdir(path_p, mode)

mbkm_train_data = train_data.copy()
miniBatchKMeans = MiniBatchKMeans(init="k-means++", n_clusters=2)
#result_df = pd.DataFrame(columns=('column_index', 'compare_column_index', 'score', 'cluster_centers'))
for i in range(46, c_num):
    c1 = transformed_x[:,i]
    print('======================================================= ' + str(i))
    if i < (c_num - 1) :
        for j in range(i+1,c_num):
            ...
            for id in _ids:
                ...
                result_output_x.Conscientious.loc[result_output_x.pId == id] = highest_confidet
