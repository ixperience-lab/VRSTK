    ...
    namespace Scripts
    {
        namespace Telemetry
        {
            ...
            public class EventSender : MonoBehaviour
            {
                ...
                private void Update()
                {
                    if (timedInterval && TestStage.GetStarted())
                    {
                ...
                public void Deploy()
                {
                    ...
                    eventToSend.time = TestStage.GetTime();
                    EventReceiver.ReceiveEvent(eventToSend);
                    eventToSend = Instantiate(eventBase);
                }
                            ...
                            for (int j = 0; j < vars[i].Length; j++)
                            {
                                if (vars[i][j])
                                {
                                    if (j >= trackedComponents[trackedCompsIndex].GetType().GetProperties().Length)
                                    {
                                        ...
                                        eventBase.AddParameter(eventVarNames[eventVariableIndex], EventTypeChecker.getIndex(trackedComponents[trackedCompsIndex].GetType().GetField(trackedVariableNames.array[trackedCompsIndex].array[varNameIndex]).GetValue(trackedComponents[trackedCompsIndex]).GetType()));
                                    }
                                    else
                                    {
                                        ...
                                        eventVariableNames.array[trackedCompsIndex].array[varNameIndex] = eventVarNames[eventVariableIndex];
                                        eventBase.AddParameter(eventVarNames[eventVariableIndex], EventTypeChecker.getIndex(trackedComponents[trackedCompsIndex].GetType().GetProperty(trackedVariableNames.array[trackedCompsIndex].array[varNameIndex]).GetValue(trackedComponents[trackedCompsIndex]).GetType()));
                                    }
                                    varNameIndex++;
                                    eventVariableIndex++;
                                }
                ...
                public Component GetComponentFromParameter(string parameterName)
                {
                    ...
                    return GetComponent<EventSender>();
                }
