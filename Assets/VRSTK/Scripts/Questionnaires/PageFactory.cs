...
namespace VRSTK
{
    namespace Scripts
    {
        namespace Questionnaire
        {
            ...
            {
                [SerializeField]
                private List<GameObject> _questionList; //contains all questions which belong to one page
                public List<GameObject> QuestionList //contains all questions which belong to one page
                {
                    get { return _questionList; }
                    set { _questionList = value; }
                }

                public GameObjectList(List<GameObject> questionList)
                {
            ...
            public class PageFactory : MonoBehaviour
            {
                ...
                //[SerializeField]
                //private List<GameObject> _temp; //contains all questions which belong to one page

                [SerializeField]
                private List<GameObjectList> _questionList; //contains all questions which belong to one page

                public List<GameObjectList> QuestionList //contains all questions which belong to one page
                {
                    get { return _questionList; }
                    set { _questionList = value; }
                }
                ...
                */
                    ...
                    switch (qType)
                    {
                        ...
                        case "radioGrid":
                            if (qConditions.Count < 8)
                            {
                                for (int i = 0; i < qConditions.Count; i++)
                                {
                                    ...
                                    SetRec(radioHorizontalRec);
                                    _questionList.Add(new GameObjectList(temp.GetComponent<VRQuestionnaireToolkit.RadioGrid>().CreateRadioGridQuestion(qId, qType, qInstructions, _qData[0][0],
                        ...
                        case "checkbox":
                            ...
                            for (int i = 0; i < _qData.Count; i++)
                            {
                                ...
                                _questionList.Add(new GameObjectList(temp.GetComponent<VRQuestionnaireToolkit.Checkbox>().CreateCheckboxQuestion(qId, qType, qInstructions, _qData[i][0], _qData[i][1], qOptions, i, radioHorizontalRec)));
                            }
                            break;
                        case "checkboxGrid":
                        ...
                        case "linearGrid":
                            for (int i = 0; i < _qData.Count; i++)
                            {
                                ...
                                _questionList.Add(new GameObjectList(temp.GetComponent<VRQuestionnaireToolkit.LinearGrid>().CreateLinearGridQuestion(qId, qType, qInstructions, _qData[i][0], _qData[i][1], qOptions, i, radioHorizontalRec, _qData[i][2].AsBool, _qData[i][3], _qData[i][5], _qData[i][4], _qData[i][6])));
                            }
                            break;
                        case "linearSlider":
                            for (int i = 0; i < _qData.Count; i++)
                            {
                                ...
                                _questionList.Add(new GameObjectList(temp.GetComponent<VRQuestionnaireToolkit.Slider>().CreateSliderQuestion(qId, qType, qInstructions, _qData[i][0], _qData[i][1], qOptions, i, radioHorizontalRec, _qData[i][2], _qData[i][4], _qData[i][3], _qData[i][5])));
                            }
                            break;
                        case "dropdown":
                            for (int i = 0; i < _qData.Count; i++)
                ...
                public void GenerateAndDisplayFirstAndLastPage(bool firstPage, string qText, string qInstructions)
                {
                    ...
                    if (firstPage)
                    {
                        ...
                        _questionList = new List<GameObjectList>();
