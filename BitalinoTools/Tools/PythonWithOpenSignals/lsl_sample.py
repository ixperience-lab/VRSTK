    ...
    def recv_data_unspecified_OS_stream(self, client):
        ...
        try:
            while True:
                ...
                if len(sample) == 3:
                    message = "{0} - Seq[{1}] : O[false false false false] ; A[{2} {3} 0 0 0 0]".format(timestamp, int(sample[0]), int(sample[1]), int(sample[2]))
                if len(sample) == 4:
                    message = "{0} - Seq[{1}] : O[false false false false] ; A[{2} {3} {4} 0 0 0]".format(timestamp, int(sample[0]), int(sample[1]), int(sample[2]), int(sample[3]))
                if len(sample) == 5:
                    message = "{0} - Seq[{1}] : O[false false false false] ; A[{2} {3} {4} {5} 0 0]".format(timestamp, int(sample[0]), int(sample[1]), int(sample[2]), int(sample[3]), int(sample[4]))
                if len(sample) == 6:
